...
// Rotas de auth (sem proteção - login/signup são públicos)
app.use('/api', authRouter);

// Rotas da API protegidas por login
...
async function start() {
  ...
  const server = app.listen(PORT, () => {
    console.log(`\n✂️  CutTools v4.2 rodando em http://localhost:${PORT}`);
    console.log(`   Contas: ${stats.accounts} | Posts: ${stats.total} | Pendentes: ${stats.pendente}`);
    console.log(`   🔐 Login habilitado — Admin: ${process.env.ADMIN_EMAIL || 'admin@admin.com'}\n`);
    ig.restoreJobs();
  });
